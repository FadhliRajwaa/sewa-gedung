...
try {
    // Total riwayat (pemesanan selesai atau lunas)
    $query = "SELECT COUNT(*) as total_riwayat 
              FROM pemesanan p
              LEFT JOIN pembayaran pb ON p.id_pemesanan = pb.id_pemesanan
              WHERE (pb.status_pembayaran = 'Lunas' OR p.tanggal_sewa < CURDATE())";
    ...
    $query = "SELECT COUNT(*) as bulan_ini 
              ...
              LEFT JOIN pembayaran pb ON p.id_pemesanan = pb.id_pemesanan
              WHERE (pb.status_pembayaran = 'Lunas' OR p.tanggal_sewa < CURDATE())
              AND MONTH(p.tanggal_sewa) = MONTH(CURRENT_DATE()) 
              AND YEAR(p.tanggal_sewa) = YEAR(CURRENT_DATE())";
    $stmt = $pdo->prepare($query);
    $stmt->execute();
    $bulanIni = $stmt->fetch(PDO::FETCH_ASSOC)['bulan_ini'] ?? 0;
    
    // Total pendapatan dari riwayat
    $query = "SELECT SUM(p.total) as total_pendapatan 
              FROM pemesanan p
              LEFT JOIN pembayaran pb ON p.id_pemesanan = pb.id_pemesanan
